...
| Date | Author | Change |
|---|---|---|
| 2026-09-01 | Lucas Manoukian | **Corrección posterior al merge de `T-1.30`: tres bugs visuales reales, encontrados mirando la pantalla y no por el checklist.** El hallazgo `(5)` de la entrada de abajo decía "sin hallazgos que graduar en `R-01`/`R-02`" — quedó desmentido apenas se miró con más atención. **(1)** `.team-panel.negro .icon-boot{ filter: brightness(0) invert(1) }` (rebanada 3) invierte la pelota a blanco para leerse sobre el panel oscuro; el chip nuevo es un pill CLARO también en el equipo Negro, así que ahí la pelota invertida casi no se veía. Se restituyó el ícono normal dentro de `.stat-pill` con `.team-panel.negro .stat-pill .icon-boot{ filter: none }`. **(2)** Los chips de asistencias y de goles, cada uno anclado a su propio borde de la camiseta con los valores literales del handoff (`--stat-edge`/`--stat-pad`), se solapaban cada vez que una unidad tenía las dos cosas a la vez — no sólo el límite triple (gol + en contra + asistencia) que el handoff ya reconocía, sino el caso común de un solo gol y una sola asistencia: medido, hasta 5-6px de solapamiento con una camiseta de 44 a 52px, que es el tamaño que se renderiza en la práctica casi siempre (la container query de `--chip-size` dispara su versión compacta con cualquier panel de dos columnas, no sólo en pantallas angostas). Se resolvió envolviendo los dos grupos en un `flex` centrado (`.stat-chips`) con un `gap` real entre ambos, activo sólo cuando los dos coexisten. **(3)** Ese centrado, aplicado sin condición, corría también el chip de una unidad con un solo tipo de evento al centro de la camiseta, perdiendo el ancla a su propio borde que pedía el handoff (y que un administrador pidió de vuelta al ver la pantalla). Se acotó: `renderCamiseta` envuelve en `.stat-chips` sólo cuando existen los dos fragmentos; con uno solo, ese chip queda como hijo directo de `.camiseta-fig` y `.camiseta-fig > .stat-asistencias` / `> .stat-goles` lo vuelve a anclar a la izquierda o a la derecha. Se agregó `INVARIANTE_CHIPS_ESTADISTICA` a `tests/layout.test.js` —compara los dos grupos DENTRO de la misma camiseta, algo que `INVARIANTE_CANCHA` no hace porque sólo mide camisetas vecinas— y un titular de `m-finalizado-nueve` con gol y asistencia a la vez en la fila de cuatro (chip-size 48px), para que el `(2)` no pueda volver sin que un test lo vea. Cuatro commits nuevos, ninguno en el checklist de `§7.2.9` porque ninguno estaba previsto: dos `fix(finalizado)` de código, uno `test(layout)`, y éste. |
