...
| OPEN-Q-03 | Valores exactos del escalón derivado para el ancho útil más chico | Lucas Manoukian | **Branch 1**, `T-1.25` | El Plan fija el método, no los números: se miden y se ajustan hasta que la cancha de 9 entre a 360 px |
| OPEN-Q-04 | ¿Cuál es el ancho útil real del panel a 360 px? El Concept Note dice 312 px; la aritmética sobre los valores del CSS da 296 px | Lucas Manoukian | **Branch 1**, `T-1.26` | Se mide una vez en el navegador y se anota en el PR. Cierra también el `[UNVERIFIED]` de `TC-013` |
| OPEN-Q-05 | Anotación recíproca en las dos specs de `.specify/` parcialmente reemplazadas | Lucas Manoukian | **Branch 1**, `T-1.28` | El Principio I pide la declaración de los dos lados; la mitad de la Spec ya está hecha |
...
| OPEN-Q-08 | `[UNVERIFIED]` heredado de la Spec `AC-15`: no se leyó si el doble de `tests/fixtures-app.js` permite observar las escrituras | Lucas Manoukian | **Branch 1**, `T-1.16` | Se cierra al implementar `window.__escrituras`; si el doble no lo permitiera, la evidencia alternativa es revisión del diff de las llamadas de guardado |
