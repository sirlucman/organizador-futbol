...
| TD-10 | **(2026-09-01, post-merge, ver Change log)** La mantención sostenida (`FR-054`) usa `pointerdown`/`pointerup`/`pointercancel`/`pointerleave` — un solo juego de eventos, no uno para touch y otro para mouse — y suprime el click que sigue con una bandera de módulo (`ultimoToqueFuePresionSostenida`), no con `preventDefault` | `FR-054`, `FR-054b` | Pointer Events unifica touch/mouse/pen en el mismo navegador desde hace años, así que no hace falta duplicar el cableado ni medir a mano si el dedo se movió: el propio navegador dispara `pointercancel` cuando decide que el gesto es para scrollear. La bandera en vez de `preventDefault` es lo que mantiene testeable el gesto con clicks sintéticos (Playwright), que no siempre pasan por el pipeline completo de eventos de puntero |
...
| Date | Author | Change |
|---|---|---|
| 2026-09-01 | Lucas Manoukian | Post-merge: se agrega la mantención sostenida (`FR-054`/`FR-054b` de la Spec, `TD-10`) — mantener presionado el mismo destino de toque saca un evento de la familia activa, en vez de agregar uno. Implementado sobre lo que ya existía: `window.__presionarInicio`/`window.__presionarFin` arman/cancelan un `setTimeout` de 550ms y, al completarse, llaman a `__quitarUltimoDeFamiliaCarga` (sin código de negocio nuevo); `window.__tocarNombreJugador` descarta el click que sigue a una mantención ya resuelta con una bandera de módulo. Cubierto por `tests/layout.test.js` invocando los handlers directamente (`toque/S-04d`, `toque/S-04e`) — un `setTimeout` real de 550ms no es practico de esperar en la suite completa (13 anchos × 30+ escenarios), así que el test llama `window.__presionarInicio` y lee el resultado sin pasar por el temporizador de verdad. Self-critique: no corresponde (adición acotada, con el código y el test ya verificados). |
