...
| OPEN-Q-07 | El escenario `tooltip` de `layout.test.js` mide una pantalla sin ningún `.info-icon`: el `?` del combo era el único de la aplicación y `D-14` lo retiró. ¿Se elimina el escenario, o se conserva porque el mecanismo puede volver? | Lucas Manoukian | Revisión posterior al merge | Levantado por la implementación. El escenario **pasa en verde midiendo nada**, que es peor que fallar. No se elimina en esta rama porque retirar cobertura de layout es una decisión de test y no de esta rebanada; queda anotado para decidirlo con el dueño. El CSS de `.info-icon` y `.info-tip` sí se retiró, que es lo que `TC-014` pide |
...
| 2026-09-01 | Lucas Manoukian | **Registro de lo que la implementación encontró**, para que el Plan siga describiendo lo que pasó y no lo que se planeó. Seis hallazgos. **(1)** El fixture de layout llevaba `formacion` sólo con `objetivo`, y el resumen lee `formacion.blanco.cumplida` sin guarda: agregarlo a medias rompía el render entero de la tarjeta. **(2)** La posición asignada del fixture salía de `i % 8 === 0 ? 'Arquero' : principal`, sin mirar si el jugador tenía puntaje en esa posición — el Arco y el Ataque sumaban cero en los dos equipos y sus celdas no se dibujaban, así que el fixture no podía ejercitar el caso que distingue `D-22`. Se reescribió para llenar cada lugar con alguien que puntúe ahí y para llenar las líneas de un solo lugar primero, como hace el motor. Con el plantel testigo eso deja el Arco y el Ataque desparejos por 6 y las dos líneas de campo por 4: el caso exacto, y salido del plantel real. **(3)** El `?` del combo era el **único** `.info-icon` de la aplicación; al retirarlo por `D-14`, el mecanismo de tooltip quedó sin productores y el escenario `tooltip` de `layout.test.js` pasó a medir una pantalla vacía — ver `OPEN-Q-07`. **(4)** El botón de copiar no podía ser exclusivo de `admin` como decía `FR-001`: el rol `jugador` ya lo tenía. **(5)** Al extraer `explicacionesDelArmado` apareció que ordenar por posición no es cosmético — `agruparFilasDeEquipo` reconoce una dupla sólo si sus integrantes quedan adyacentes—, y el helper subió a ámbito de módulo como `jugadoresDeEquipoOrdenados`. **(6)** El navegador conducido deniega el portapapeles por defecto, así que `S-07a`/`S-07b` se prueban con el permiso denegado y `S-07`/`S-07c` otorgándolo. Se agregaron además `estrategiaValida`, `explicacionesDelArmado` y cinco declaraciones más a la lista del arnés, todas encontradas por el test y no por revisión. **Desvío de commits:** los nueve commits de §7.2.9 se ejecutaron como seis; `T-1.C2` a `T-1.C6` quedaron agrupados en uno solo porque se hicieron sobre el mismo árbol y partirlos después habría sido reescribir historia sin ganancia. |
