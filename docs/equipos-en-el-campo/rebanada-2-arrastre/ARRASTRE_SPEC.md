...
- `S-01a [boundary]` — el jugador arrastrado es el único de su línea: la línea de origen se sigue dibujando, vacía, en su lugar (`FR-012` de la rebanada 1, enmendado 2026-09-02 — ver CANCHA_SPEC.md §18: omitirla hacía que la línea de al lado subiera a ocupar ese espacio)
...
| OPEN-Q-02 | ¿El arrastre necesita un equivalente alcanzable sin gesto de puntero (teclado, o un menú sobre la camiseta con "pasar al otro equipo")? | Lucas Manoukian | Spec revision o rebanada 6 | El arrastre es intrínsecamente un gesto de puntero y no tiene equivalente de teclado. La alternativa disponible sigue siendo regenerar. No bloquea: no hay regresión respecto de lo que la aplicación ofrecía antes de la rebanada 1. El selector, en cambio, **sí** es alcanzable por teclado por ser dos botones |
| OPEN-Q-03 | *Resuelta.* ¿Los resúmenes de la tarjeta (diferencia de puntaje, diferencia por línea, conteo de posiciones) se recalculan hoy tras un movimiento manual? Y si no, ¿se arregla en la rebanada 3? | Lucas Manoukian | *Resuelta* | El Plan midió el comportamiento vigente (tres de cuatro sí, la diferencia por línea no). La segunda mitad la cerró `D-25` del Concept Note: **sí se arregla en la rebanada 3**, y sólo para los números — el receipt sigue describiendo la última generación. Ver [`PANEL_ARMADO_SPEC.md`](../rebanada-3-panel-armado/PANEL_ARMADO_SPEC.md) `FR-070` a `FR-074` |
...
| OPEN-Q-05 | *Resuelta.* Con el selector ya construido, ¿la rebanada 3 lo conserva donde está, lo mueve, o lo extiende a las otras pantallas que el handoff también dibuja con selector (`6b`, partido finalizado)? | Lucas Manoukian | *Resuelta* | Cerrada por la [Spec de la rebanada 3](../rebanada-3-panel-armado/PANEL_ARMADO_SPEC.md) §3.2: **lo conserva donde está** —entre el combo y la cancha, que es donde el handoff lo dibuja— y no lo extiende a ninguna otra pantalla. Las vistas `6b` y `8d` siguen anotadas para las rebanadas 4 y 6 |
