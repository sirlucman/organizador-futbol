...
| TD-03 | Mientras se espera el refresco del token se muestra el **loader de pelota que ya existe** (`renderBallLoader`, [`index.html:7061`](../../index.html#L7061)) dentro de `.login-screen`, en lugar de `.login-card`. Sólo se muestra si el claim falta; con token vigente no se pinta nunca | FR-006, NFR-001b, NFR-006 (corregido — ver §15.1 `OPEN-Q-06`) | Decisión del propietario. Evita que la pantalla de login parezca colgada los ~250 ms del refresco, y reusa el componente del design system en vez de inventar uno (`AGENTS.md` → design system como fuente de verdad). Es un estado de layout nuevo y por eso suma escenario propio en [`tests/layout.test.js`](../../tests/layout.test.js) |
...
|---|---|---|---|---|---|---|
| 0 | `feat/rol-en-el-token` | `main` | En curso | — | — | Rama de documentos (Concept, Spec, este Plan). El nombre usa el prefijo `feat/` y no el `docs/` que fija `AGENTS.md`; se deja como está para no reescribir historia, y se anota como desvío |
| 1 | `feature/rol-en-el-token-script` | `main` | Not started | — | `rol-script`, `reglas` | Fase `scaffolding`. Cero cambio en `index.html`. Su **corrida** contra los dos proyectos es requisito de merge de la Rama 2 |
| 2 | `feature/rol-en-el-token-app` | `main` | Not started | — | `sesion`, `layout` | Fase `core`. Requiere que la Rama 1 esté mergeada **y corrida** |
| 3 | `feature/rol-en-el-token-reglas` | `main` | Not started | — | `reglas`, `layout` | Fase `rollout`. Requiere que la Rama 2 esté mergeada y publicada. Incluye publicar reglas a mano en prod y staging |
...
| `mostrarLoaderSesion` | `() -> void` | Nueva. Oculta `.login-card` y monta `renderBallLoader({ size: 92, caption: 'Entrando', shadow: true })` en `.login-screen` (TD-03) |
| `ocultarLoaderSesion` | `() -> void` | Nueva. Deshace lo anterior, para que un logout y un login nuevo encuentren la tarjeta en su lugar |
...
| `tests/layout.test.js` | Escenarios nuevos: `rol-admin-primer-pintado` (`rol/S-01`, `rol/S-01d`, `rol/S-02`, `rol/NFR-001`, `rol/NFR-002`, `rol/NFR-007`) · `rol-sesion-loader` (`rol/S-01b`, `rol/S-11`, `rol/NFR-001b`, `rol/NFR-006`) · `rol-jugador-primer-pintado` (`rol/S-03`) · `rol-login-fallido` (`rol/S-02a`) · `rol-dos-pestanias` (`rol/S-02b`) |
...
- [ ] Con `claimAusente: true, refrescoTrae: 'admin'`, se ve el loader, se pide **un** refresco y la app aparece con las tres solapas (S-11, S-11a)
...
- [ ] El escenario `rol-sesion-loader` de `layout.test.js` **se vio fallar** al menos una vez, revirtiendo el loader, antes de darlo por bueno (`AGENTS.md` → Tareas)
- [ ] `node tools/medir-arranque.js --caso=vigente` contra staging: hueco ≤ 50 ms (AC-10, NFR-001)
- [ ] `node tools/medir-arranque.js --caso=vencido` contra staging: hueco + retención del loader ≤ 400 ms en la mediana de tres corridas (AC-11, NFR-001b)
...
- [ ] T-2.4 Agregar el marcado del loader de sesión a `.login-screen` en [`index.html:1087-1103`](../../index.html#L1087-L1103) y las funciones `mostrarLoaderSesion` / `ocultarLoaderSesion`, reusando `renderBallLoader` (TD-03, FR-006)
- [ ] T-2.5 [P] Agregar el CSS del loader de sesión con tokens del design system, sin valores inventados (§5 *Design system*)
...
- [ ] T-2.15 Agregar el escenario `rol-sesion-loader` a [`tests/layout.test.js`](../../tests/layout.test.js) y **verlo fallar** revirtiendo el loader, antes de nada más (`AGENTS.md` → Tareas; NFR-006)
...
- [ ] T-2.D1 Pasan los tests nuevos — `node tests/sesion.test.js` y `node tests/layout.test.js --solo=rol-sesion-loader`
...
| OBS-03 | Salida de `node tools/medir-arranque.js`: hueco de la solapa, retención del loader y conteo de `get` por colección | metric (a demanda) | `tools/medir-arranque.js` | NFR-001, NFR-001b, NFR-002, NFR-004, NFR-007 | NFR-001 ≤ 50 ms con token vigente; NFR-001b ≤ 400 ms de hueco + retención, mediana de tres, con token vencido; NFR-002 exactamente 0 `get` de `userRoles`. Es la señal **primaria** de los cuatro NFR cuantificados, y es la que NFR-007 exige para que no se verifiquen por inspección de código |
...
| S-01b `[boundary]` token vencido | `tests/layout.test.js` escenario `rol-sesion-loader` (`spec: ['rol/S-01b']`) | e2e | Rama 2 |
...
| S-11 (parent) el corte: sesión abierta antes del cambio | `tests/layout.test.js` escenario `rol-sesion-loader` (`spec: ['rol/S-11']`) | e2e | Rama 2 |
...
| R-09 | La retención de ~250 ms del loader con token vencido se siente como un cuelgue en un teléfono con datos móviles, donde el refresco puede costar bastante más que la mediana medida | med | low | `OBS-01` (el campo `ms`), `OBS-03` (`--caso=vencido`) | `T-2.4`, `T-2.18` | monitored only — see OBS-01. Si el `ms` observado en uso real se va muy por encima de los 400 ms de NFR-001b, la alternativa es revisar TD-02 con el propietario: mostrar la app antes y aceptar el reacomodo, que exige corregir FR-002 |
...
| AC-11 | Rama 2 (y se re-mide en Rama 3) | `node tools/medir-arranque.js --caso=vencido` (`T-2.19`, `T-3.9`), mediana de tres. Gate mecánico: escenario `rol-sesion-loader`, etiqueta `rol/NFR-001b`. Ver §12.8 sobre por qué se mide hueco **+** retención |
...
| AC-23 | Rama 2 | `tests/layout.test.js` escenario `rol-sesion-loader` (`"rol/S-11"`) + `tests/sesion.test.js` `"rol/S-11a"` |
