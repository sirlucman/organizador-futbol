...
| Commits | **Conventional Commits con el asunto en español**: `tipo(scope): asunto en minúscula, ≤ 72 caracteres (IDs de la Spec)`. Tipos: `feat`, `fix`, `docs`, `test`, `refactor`, `style`, `chore`. Scope de esta feature: `rol-en-el-token` (o `tests` / `tools` cuando el cambio es sólo de esa zona). Asunto en imperativo o infinitivo, sin punto final. Un cambio lógico por commit; cada commit pasa `node --check` y sus tests por separado, para que `git bisect` sirva. **Anotado al implementar:** el límite de 72 caracteres no se cumple, y no por descuido — los mensajes que este mismo Plan prescribe en sus tareas `T-N.C*` miden entre 73 y 88 caracteres. Con el prefijo `refactor(rol-en-el-token): ` (27 caracteres) más los IDs de la Spec entre paréntesis, quedan menos de 40 para decir algo. El historial del repositorio tampoco lo cumple: 22 de los últimos 60 commits de `main` lo superan, con un máximo de 117. Se dejaron los mensajes tal como los prescribe el Plan, que es lo que los vuelve rastreables tarea por tarea, y la discrepancia queda acá en vez de pasar en silencio por el gate `T-N.D11`. Si el límite importa, lo que hay que cambiar es el scope o el límite, en `AGENTS.md` |
...
- [x] `node tools/medir-arranque.js --caso=vencido` contra staging: arranque completo ≤ 2000 ms en la mediana de tres corridas (AC-11, NFR-001b). **Medido el 2026-09-10: mediana 497 ms** (470–498). El objetivo era 400 y se subió a 600 por esta misma medición — ver el change log de la Spec. **Vuelto a medir el 2026-09-10 con el arreglo del arranque en dos etapas: mediana 1693 ms** (1444–1698), y el objetivo pasó a 2000: la barra de solapas ahora se pinta junto con los datos, así que la magnitud incluye la lectura a Firestore. La espera de la persona no cambió (contenido pintado: 625 ms antes, 613 ms después)
...
- [x] Reglas publicadas en **producción** el 2026-09-10, verificadas con las **dos** cuentas reales. El lado *deny* de producción **sí se pudo probar**, contra lo que anticipaba `OPEN-Q-07`: en vez de necesitar la contraseña de la cuenta `jugador`, el Admin SDK emite un *token personalizado* para su uid y se lo canjea por un ID token, que llega con los custom claims de la cuenta. Sirve para cualquier cuenta de cualquier proyecto sin conocer su contraseña, y no modifica nada
...
| OBS-03 | Salida de `node tools/medir-arranque.js`: hueco de la solapa, arranque completo y conteo de `get` por colección | metric (a demanda) | `tools/medir-arranque.js` | NFR-001, NFR-001b, NFR-002, NFR-004, NFR-007 | NFR-001 ≤ 50 ms con token vigente; NFR-001b ≤ 2000 ms de arranque completo, mediana de tres, con token vencido (objetivo subido de 400 a 600 y después a 2000, las dos veces el 2026-09-10 y por medición: la segunda al pasar la barra de solapas a pintarse junto con los datos); NFR-002 exactamente 0 `get` de `userRoles`. Es la señal **primaria** de los cuatro NFR cuantificados, y es la que NFR-007 exige para que no se verifiquen por inspección de código |
...
- ~~**El lado *deny* en producción** para `data/ordenJugadoresMigrado`~~ → **verificado el 2026-09-10** con un token personalizado emitido por el Admin SDK, que no requiere la contraseña de la cuenta. `OPEN-Q-07` resuelta
...
| NFR-001b | Arranque completo ≤ 2000 ms con token vencido, mediana de tres | `node tools/medir-arranque.js --caso=vencido`, tres corridas (`OBS-03`, AC-11). **La magnitud es el arranque completo** —desde que `onAuthChange` entrega la cuenta hasta que la barra queda pintada— y no el hueco de la solapa: con `TD-02` el hueco es 0 ms en los dos casos y medirlo no discriminaría nada. **Medido el 2026-09-10: mediana 497 ms**, con lo que el objetivo pasó de 400 a 600 ms; los 400 salían del refresco medido aislado (250 ms) y no del camino completo. En la misma corrida el caso normal dio **12 ms** contra un objetivo de 50. **Vuelto a medir el 2026-09-10** con el arreglo del arranque en dos etapas: mediana **1693 ms** en el corte y **809 ms** en el caso normal, y el objetivo pasó de 600 a 2000 — la barra ahora se pinta junto con los datos, así que la magnitud absorbió la lectura a Firestore. Con la lectura adentro este número dejó de aislar una regresión en la resolución del rol: para eso quedan `TC-046` y el presupuesto de sobrecosto del escenario `rol-corte-token-vencido`, que corre contra un doble sin latencia |
...
| ~~OPEN-Q-07~~ | ~~¿Deniega producción la lectura de `data/ordenJugadoresMigrado` a una cuenta `jugador`?~~ | Lucas Manoukian | ~~Rama 3 (§12.7)~~ | **Resuelta el 2026-09-10 por medición: sí, deniega.** Se creía imposible de probar por no haber contraseña de una cuenta `jugador` de producción, y la premisa era falsa: el Admin SDK —cuya llave ya se necesita para estampar— emite un token personalizado para cualquier uid, y al canjearlo se obtiene un ID token con los custom claims de esa cuenta. Con eso se verificaron en producción las lecturas de los nueve documentos con los dos roles y las escrituras denegadas del rol `jugador` sobre los siete que no le corresponden. Queda como método reusable para cualquier verificación futura de reglas en un proyecto sin credenciales de ingreso |
...
| 2026-09-10 | Lucas Manoukian (claude-opus-5) | **Feature entregada.** Las tres ramas mergeadas a `main` y publicadas — la feature entra en **v2.0.12** y la versión publicada quedó en **v2.0.13**, porque el versionado de este repositorio es **automático** (`.github/workflows/version-bump.yml`) y el bump manual que se hizo al mergear sobraba; queda documentado en [`AGENTS.md`](../../AGENTS.md) → *Versionado*, roles estampados en los dos proyectos y reglas nuevas publicadas en los dos. Verificación de producción con las **dos** cuentas: lecturas de los nueve documentos por los dos roles, escrituras denegadas del rol `jugador` sobre los siete que no le corresponden, `userRoles` denegado para ambos, y el lado *allow* de admin comprobado reescribiendo el **mismo valor** en un flag de migración —sin cambiar ningún dato—. Con eso queda resuelta `OPEN-Q-07`, que se daba por imposible: no hacía falta la contraseña de la cuenta `jugador` de producción, alcanza un token personalizado emitido por el Admin SDK. Pendientes menores fuera del alcance del Plan: la cuenta `rol-test@organizador-futbol.local` que `tests/reglas.test.js` crea en staging, y la limitación preexistente de `data/playersSortMode` (una cuenta `jugador` puede cambiar el orden del listado y esa escritura falla en silencio), anotada en el contrato §2.3 como decisión de producto. |
