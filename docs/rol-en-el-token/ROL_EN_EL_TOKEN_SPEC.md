...
| NFR-006 | Compatibility | La feature introduce **un** estado de layout nuevo —el **loader de sesión**, que tapa la espera del refresco del token dentro de `.login-screen`— y ese estado deberá tener escenario propio en `tests/layout.test.js`, medido desde el piso de 360 px de [`AGENTS.md`](../../AGENTS.md). Ningún otro: la barra con tres solapas ya existe hoy para un administrador y se cumple por no-regresión. **Corregido el 2026-09-09** (ver §18): la redacción original decía que la feature no introducía ningún estado nuevo, y el Implementation Plan la dejó desactualizada al decidir en `TD-03` tapar el refresco con el loader de pelota que ya existe. Se corrige acá y no en el Plan porque un requisito vigente no puede contradecir su propia implementación |
...
- **AC-11** — NFR-001b verificado midiendo el **hueco de la solapa más la retención del loader de sesión** con una sesión de más de una hora, y comprobando que la mediana de tres corridas es ≤ 400 ms y menor que la línea de base de 487 ms. **Corregido el 2026-09-09** por la misma causa raíz que NFR-001b: medir sólo el hueco, que `TD-02` vuelve 0 ms por construcción, no verificaría nada.
...
| Hueco de la solapa, token vencido | ≤ 400 ms (mediana), desde ~750 ms | Igual, con una sesión de más de una hora. El ~750 ms de partida es refresco (mediana 250) + lectura a Firestore (mediana 499), ambos medidos por separado el 2026-09-09 |
