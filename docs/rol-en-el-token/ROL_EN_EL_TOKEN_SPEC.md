...
| La reescritura de reglas omite `ordenJugadoresMigrado` porque la fuente de verificación committeada no lo incluye, y ese documento queda con su regla vieja —haciendo el `get()` que esta feature elimina— mientras los otros cinco migran | Med | Med | `TC-041` ya no admite el contrato committeado como fuente: exige verificar contra las reglas vivas de los dos proyectos. `OPEN-Q-04` confirmó por medición que en staging esa regla existe y funciona, así que el riesgo dejó de ser "puede no haber ninguna" y pasó a ser sólo el de omitirla en la reescritura. Sigue en Med porque la única fuente de su texto es la consola |
...
| OPEN-Q-04 | ¿Qué regla rige **hoy** el documento `data/ordenJugadoresMigrado` en cada proyecto Firebase? | Lucas Manoukian | Implementation Plan | **Resuelta para staging el 2026-09-09; pendiente para producción.** Detectada por la crítica independiente: el contrato committeado de `007` no tiene bloque `match` para ese documento, aunque la app lo pide como sólo-admin desde `orden-jugadores`. Probado empíricamente contra staging con una sonda de solo lectura (login real, SDK de Firebase, sin cargar la app para no disparar su migración): un `admin` **lee** el documento y su valor es `true`; un `jugador` recibe `permission-denied`, igual que en los otros cinco documentos sólo-admin. Conclusión para staging: **existe una regla, está correctamente restringida a admin, y el flag persistió** — con lo que queda descartado el escenario de que la migración estuviera re-corriendo y reseteando el orden del plantel en cada arranque. Lo que sigue abierto: (a) el **texto** de esa regla, que no se puede leer desde el cliente y hay que copiar de la consola para reescribirla junto con las otras cinco (`TC-041`); y (b) si **producción** se comporta igual, que no se pudo probar porque las cuentas de Auth son por proyecto y las credenciales disponibles son de staging. El riesgo que queda es de omisión, no de agujero: ver §15 |
...
| 2026-09-09 | Lucas Manoukian (claude-opus-5) | Resuelve `OPEN-Q-04` para staging con una medición empírica, y deja pendiente sólo producción. Sonda de solo lectura contra el proyecto de staging (login real con el SDK de Firebase, sin cargar `index.html`, deliberadamente, para no disparar la migración de `orden-jugadores` durante la prueba): un `admin` lee `data/ordenJugadoresMigrado` y su valor es `true`; un `jugador` recibe `permission-denied`, igual que en los otros cinco documentos sólo-admin. Conclusión: existe una regla, está correctamente restringida a admin y el flag persistió, con lo que **queda descartado** el peor escenario que se había planteado — que la escritura del flag estuviera fallando en silencio y la migración reseteara el orden del plantel a alfabético en cada arranque de admin. El riesgo de §15 se recalibra de `High` a `Med`: dejó de ser "puede no existir ninguna regla" y pasó a ser sólo el de omitirla en la reescritura, porque su texto sigue siendo ilegible desde el cliente. `TC-041` no cambia: sigue exigiendo verificar contra las reglas vivas. En paralelo se anotó la deriva en el contrato de `007` ([`contracts/firestore-rules.md`](../007-permisos-por-usuario/contracts/firestore-rules.md)), que es donde estaba la causa raíz: el bloque documenta cinco de los seis documentos sólo-admin. Self-critique: no corresponde (incorporación de una medición, verificada por ejecución). |
