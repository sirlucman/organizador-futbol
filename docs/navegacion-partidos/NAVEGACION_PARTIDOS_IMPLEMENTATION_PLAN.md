...
|---|---|---|---|
| TD-01 | Sin feature flag, una sola rama de código | `D-01`..`D-07` | El proyecto no tiene infraestructura de flags y el Principio II de `.specify/memory/constitution.md` v2.4.0 prohíbe anticiparla (ese archivo se retiró el 2026-09-01; el principio, con el nombre "Simplicidad ante todo", vive hoy en [`AGENTS.md`](../../AGENTS.md)). La red de seguridad es la rama sin mergear, igual que en las cinco rebanadas ya mergeadas de "Equipos en el campo" (`TD-09` de esas Specs). |
...
| ~~TD-06~~ | ~~`.wrap` (`#appRoot`) está topeado en `max-width:760px` para toda la app — descubierto recién al implementar, no estaba en la Spec. El grid de dos columnas necesita más (352px + un panel de equipos usable no entran en 760px), así que se agrega una clase `.match-wide` que ensancha `.wrap` a 1160px sólo mientras `#matchDetailView` está abierto ≥1100px. Se agrega/quita en `renderMatchDetail`/`renderMatchList`, y también al cambiar de pestaña superior (ese tercer punto lo pedía evitar una fuga: cambiar a "Jugadores" sin pasar por "Volver a partidos" dejaba `.wrap` ancho en toda la app)~~ Superseded por `TD-06b` — ver Change log 2026-09-03: el ancho condicional hacía que la página entera (encabezado y pestañas incluidos) cambiara de tamaño al saltar de "Partidos" a otra pestaña y volver, algo que ninguno de los tres documentos había anticipado porque `FR-001` sólo hablaba del layout DENTRO de la pestaña de partidos, no del marco que la contiene | `FR-001` | Sin esto, `FR-001` es literalmente irrealizable con el ancho de columna que fija `D-01` — no es una preferencia visual, es una restricción de layout preexistente que ninguno de los tres documentos había citado |
| TD-06b | `.wrap` pasa a `max-width:1160px` siempre a partir de los 1100px de `TD-05`, sin excepción por pestaña ni por clase agregada/quitada por JS — Jugadores y Configuración usan el mismo ancho de punta a punta que Partidos, contenido incluido (no sólo el encabezado). Elimina la clase `.match-wide` y las tres llamadas que la agregaban/quitaban (`renderMatchDetail`, `renderMatchList`, el listener de pestañas). Un primer intento sólo ensanchaba `.wrap` y recentraba el contenido de esas dos pestañas en 760px dentro del marco ancho; el usuario lo vio y pidió unificar también el contenido, no sólo el encabezado (ver Change log) | `FR-001` | Ancho pedido por el usuario (2026-09-03) para que las tres pestañas midan lo mismo y no se note el salto; unifica el ancho en vez de agregar una cuarta rama a la sincronización que `TD-06` ya venía parchando (era la segunda vez que se encontraba un caso sin cubrir, ver `PARTIDO_FINALIZADO_SPEC.md`/historial de esta rama) |
...
| Date | Author | Change |
|---|---|---|
| 2026-09-03 | Lucas Manoukian | Enmienda pedida por el propietario tras usar la feature ya mergeada: las tres pestañas de la app (Jugadores, Partidos, Configuración) medían distinto en escritorio — `TD-06` sólo ensanchaba `.wrap` mientras se veía el detalle de un partido, así que saltar entre pestañas cambiaba el ancho de toda la página, encabezado incluido. Se supersede `TD-06` por `TD-06b`: ancho fijo de 1160px para las tres pestañas a partir del breakpoint. Se elimina la clase `.match-wide` y toda la sincronización JS que la mantenía (incluida la reparación del 2026-09-03 anterior a este cambio, que quedó sin efecto porque la causa de fondo — el ancho condicional — desaparece). Ningún `FR`/`NFR`/`TC` de la Spec cambia: sigue siendo el mismo layout de dos columnas, con el mismo breakpoint. **Corrección en la misma sesión, antes de commitear:** la primera versión de `TD-06b` sólo ensanchaba `.wrap` y recentraba el contenido de Jugadores/Configuración en 760px dentro de ese marco — el encabezado quedaba parejo pero el contenido de esas dos pestañas se veía más angosto que el marco que lo rodeaba. El usuario lo vio en pantalla y pidió que el contenido también usara el ancho completo, no sólo el encabezado; se saca esa regla y las tres pestañas quedan uniformes de punta a punta (`TD-06b` ya refleja la versión final). Verificado visualmente (capturas en Jugadores/Configuración a 1400px) que el contenido no queda roto ni con espacios vacíos raros al estirarse. Self-critique: no corresponde (cambio de implementación acotado a una decisión ya marcada como "valor de trabajo" en `TD-05`/`TD-06`, sin tocar ningún requisito). |
